...
export function renderPersonaBody(input: SubagentTemplateInput, hostName = 'Codex'): string {
  ...
  return [
    ...
    `  wake: ["${hostName.toLowerCase()}"] })  // signals intent; doesn't spawn a worker`,
    '```',
    '',
    ...
    `${tool('reply_email')}({ uid: 42, replyAll: true, _account: "${agent.name}",`,
    `  text: "Shipped my slice. ${hostName} — over to you for next steps.",`,
    `  wake: ["${hostName.toLowerCase()}"] })`,
    `// Bridge sits unread until the operator notices.`,
    `// Instead: bounce the baton to a teammate who can act, e.g. wake: ["<teammate-name>"].`,
    '```',
    '',
