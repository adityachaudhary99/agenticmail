...
export function renderPersonaBody(input: SubagentTemplateInput): string {
  ...
  return [
    ...
    `       a. Check open operator queries with \`${tool('call_status')}\` on the active mission, OR check the most-recent mission's metadata for an unanswered \`operator_queries\` entry.`,
    `       b. If there's an open query, post the answer via \`${tool('invoke')}({ tool: "answer_operator_query", args: { mission_id, query_id, answer } })\` (look it up with \`${tool('request_tools')}\` if not in your pre-loaded set).`,
